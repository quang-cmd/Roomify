...
import kqlhotel.bus.dichvu.DichVuBus;
import kqlhotel.entity.DichVuEntity;

public class DichVuPanel extends JPanel {

    private static final DecimalFormat MONEY_FORMAT = new DecimalFormat("#,##0");
    private static final List<String> CATEGORIES = Arrays.asList("Tất cả", "Buồng phòng", "Ăn uống", "Thư giãn", "Vận chuyển", "Tiện ích");
    ...
    private List<DichVuEntity> services = new ArrayList<>();
    ...
    private void renderGrid() {
        gridPanel.removeAll();
        List<DichVuEntity> filtered = getFilteredServices();
        for (DichVuEntity service : filtered) {
    ...
    private List<DichVuEntity> getFilteredServices() {
        if ("Tất cả".equals(selectedCategory)) {
            return services;
        }
        List<DichVuEntity> filtered = new ArrayList<>();
        for (DichVuEntity service : services) {
    ...
    private JPanel createServiceCard(DichVuEntity service) {
    ...
    private void toggleServiceStatus(DichVuEntity service) {
        ...
        if (bus.updateStatus(service.getMaDV(), nextStatus)) {
            ...
            JOptionPane.showMessageDialog(this, "Không thể cập nhật trạng thái dịch vụ.");
        }
    }

    private void showServiceDialog(DichVuEntity existing) {
        ...
        root.add(Box.createVerticalStrut(10));
        root.add(createDialogFieldGroup("Nhóm hiển thị", categoryBox));
        root.add(Box.createVerticalStrut(10));
        ...
        saveButton.addActionListener(e -> {
            if (nameField.getText().trim().isBlank() || priceField.getText().trim().isBlank()) {
                JOptionPane.showMessageDialog(dialog, "Tên dịch vụ và đơn giá là bắt buộc.");
                return;
            }
            ...
            DichVuEntity payload = editing ? existing : new DichVuEntity();
    ...
    private JLabel createStatusBadge(DichVuEntity service) {
    ...
    private String normalizeCategory(String category) {
        ...
        if (raw.contains("bu") || raw.contains("giat") || raw.contains("don phong")) {
            return "Buồng phòng";
        }
        if (raw.contains("ăn") || raw.contains("uong") || raw.contains("uống") || raw.contains("buffet") || raw.contains("mi") || raw.contains("nuoc")) {
            return "Ăn uống";
        }
        if (raw.contains("thu gian") || raw.contains("thư giãn") || raw.contains("spa") || raw.contains("massage") || raw.contains("gym")) {
            return "Thư giãn";
        }
        if (raw.contains("van chuyen") || raw.contains("vận chuyển") || raw.contains("dua don") || raw.contains("xe")) {
            return "Vận chuyển";
        }
        if (raw.contains("tien ich") || raw.contains("tiện ích")) {
            return "Tiện ích";
        }
        return category;
    }
